...
namespace selbaward
{
	...
	public:
		...
		struct CellAttributes
		{
			...
			explicit CellAttributes(const bool newInverse, const bool newDark, const bool newFlipX,
			                        const bool newFlipY) : inverse(newInverse), dark(newDark), flipX(newFlipX),
			                                               flipY(newFlipY)
			{
			}
		...
		void setTexture();
		void setTexture(const sf::Texture& texture, unsigned int numberOfTextureTilesPerRow,
		                sf::Vector2u tileSize = {8u, 8u}, sf::Vector2u textureOffset = {0u, 0u});
		...
		ConsoleScreen& operator<<(const sf::Color& newColor);
		// action depends on action flag. either adds this colour to the palette or finds the closest match in the palette. either way, sets the colour to the resulting palette id.
		...
		void setCursor(char cellChar, bool mapCharacter = true);
		// converts from character to cell value. this can map the character if the character has an associated mapping
		...
		void cyclePaletteUp(Color firstColor, Color lastColor, long int amount = 1);
		// first colour and last colour of range to cycle
		void cyclePaletteDown(Color firstColor, Color lastColor, long int amount = 1);
		// first colour and last colour of range to cycle
		...
		void paste(sf::Vector2i offset = sf::Vector2i(0, 0));
		// replace screen with last saved buffer (recalls actual cell data, not the state of the console screen e.g. cursor isn't restored)
		...
		void paste(unsigned int index, sf::Vector2i offset = sf::Vector2i(0, 0));
		// replace screen with saved buffer (recalls actual cell data, not the state of the console screen e.g. cursor isn't restored)
		void removeBuffer(unsigned int index);
		// as usual, when one buffer is removed, the indices of all following buffers are decreased
		...
		void setMappedCursorCommandCharacters(const std::string& characters,
		                                      const std::vector<CursorCommand>& cursorCommands);
	...
	private:
		...
		float m_darkAttributeMultiplier;
		// sf::Color is multiplied by this amount when the cell's dark attribute is enabled
		...
		Color priv_getModifiedColorFromColorPairUsingSpecifiedColorType(
			const ColorPair& colors, const ColorType& colorType);
		Color priv_getModifiedColorFromCellUsingSpecifiedColorType(const unsigned int& cellIndex,
		...
		void priv_modifyCellUsingPrintProperties(unsigned int index, const PrintType& printType,
	...
	enum class ConsoleScreen::Palette
	{
		...
		Colors256Sepia,
		ColorsRgb
		// 24-bit RGB colours. palette of 16,777,216 (0x1000000) colours that are ordered so that the palette index can be specified like this: RRGGBB in hex e.g. 0x00FF00 for green
	};
	...
	{
		Unused = -1,
		// unused. returns a Color created from this ColorCommand if no Color can be determined such as when an exception is thrown from a method that returns a Color but exceptions are switched off
		Opposite = -2,
		// use opposite colour (i.e. foreground color for background color and background color for foreground color)
		Invert = -3,
		// invert the opposite colour ("main colour = invert" would set main colour to inverted background colour and vice versa)
		Contrast = -4,
		// contrast the opposite colour ("main colour = contrast" would set main colour to contrast background colour and vice versa). contrast is black or white based on opposite's luminance
	};

	enum ConsoleScreen::TargetBufferCommand : int
	{
		Screen = -1,
		// targets the screen directly
		First = -2,
		// targets the first buffer if one is available, otherwise targets the screen
	...
	inline void ConsoleScreen::setTexture(const sf::Texture& texture, const unsigned int numberOfTextureTilesPerRow,
	                                      const sf::Vector2u tileSize, const sf::Vector2u textureOffset)
	{
		setTexture(texture);
	...
	inline void ConsoleScreen::setMappedCursorCommandCharacters(const std::string& characters,
	                                                            const std::vector<CursorCommand>& cursorCommands)
	{
