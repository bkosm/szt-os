...
namespace
{
	...
	const selbaward::ConsoleScreen::Cell defaultCell{
		0u, selbaward::ConsoleScreen::ColorPair(), selbaward::ConsoleScreen::StretchType::None,
		selbaward::ConsoleScreen::CellAttributes()
	};
	...
	inline sf::Color sepiaColor(const float alpha)
	{
		...
		return sf::Color((r > 255 ? 255u : static_cast<sf::Uint8>(r)), (g > 255 ? 255u : static_cast<sf::Uint8>(g)),
		                 (b > 255 ? 255u : static_cast<sf::Uint8>(b)));
	}
...
namespace selbaward
{
	...
	ConsoleScreen& ConsoleScreen::operator<<(const CursorCommand& cursorCommand)
	{
		...
		case CursorCommand::TabReverse:
			{
				...
				const unsigned int rowStartIndex{printProperties.index - col};
				int targetIndex{
					static_cast<int>(rowStartIndex + ((static_cast<long int>(col) - 1) / m_tabSize) * m_tabSize)
				};
				if ((printProperties.stretch == StretchType::Both) && (printProperties.index % m_mode.x == 0))
		...
		case CursorCommand::Backspace:
			...
			if (printProperties.stretch == StretchType::Both && printProperties.index >= m_mode.x && printProperties.
				index % m_mode.x == 0)
				printProperties.index -= m_mode.x;
			if (printProperties.index > 0)
				--printProperties.index;
	...
	ConsoleScreen::ConsoleScreen(const sf::String& texturePath, const sf::Vector2u mode)
		: m_do()
		  , m_is()
		  , m_defaultPrintProperties{
			  0u, ColorPair(defaultColor, defaultBackgroundColor), StretchType::None, CellAttributes(), Affect::Default,
		  ...
		  , m_numberOfTilesPerRow{16u}
	{
		...
		/* Custom texture handling: bart-kosmala */
		csTexture_.loadFromFile(texturePath);
		setTexture(csTexture_);
	}
	...
	void ConsoleScreen::addOverAt(const Location& location, const Cell& cell, const sf::Vector2f offset)
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
				throw Exception(
					"Cannot add stack-over cell.\nLocation (" + std::to_string(location.x) + ", " + std::
					to_string(location.y) + ") out of range.");
			return;
		}
	...
	{
		const unsigned int index{priv_cellIndex(location)};
		m_overCells.erase(std::remove_if(m_overCells.begin(), m_overCells.end(), [&index](const StackCell& stackCell)
		{
			return stackCell.index == index;
		}), m_overCells.end());
	...
	{
		const unsigned int index{priv_cellIndex(location)};
		m_underCells.erase(std::remove_if(m_underCells.begin(), m_underCells.end(), [&index](const StackCell& stackCell)
		{
			return stackCell.index == index;
		}), m_underCells.end());
	...
	void ConsoleScreen::pasteOver(const unsigned int index, const sf::Vector2i offset)
	{
		if (!priv_isScreenBufferIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot stack-paste buffer.\nBuffer index (" + std::to_string(index) +
					") out of range.");
			return;
		}
		...
		for (unsigned int i{0}; i < buffer.cells.size(); ++i)
		{
			const sf::Vector2i location{
				static_cast<int>(i % buffer.width) + offset.x, static_cast<int>(i / buffer.width) + offset.y
			};
			if (location.x < 0 || location.y < 0)
				continue;
			const sf::Vector2u cellLocation{
				static_cast<unsigned int>(location.x), static_cast<unsigned int>(location.y)
			};
	...
	unsigned int ConsoleScreen::getCellAttributesBitmask(const PrintType printType)
	{
		...
		return (attributes.inverse ? Affect::Inverse : 0u) | (attributes.dark ? Affect::Dark : 0u) | (
			attributes.flipX ? Affect::FlipX : 0u) | (attributes.flipY ? Affect::FlipY : 0u);
	}
	...
	void ConsoleScreen::clearCellAt(const sf::Vector2u location)
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot clear cell.\nLocation (" + std::to_string(location.x) + ", " + std::
					to_string(location.y) + ") out of range.");
			return;
		}

		priv_clearCell(priv_cellIndex(location), m_cursorPrintProperties.colors.foreground,
		               m_cursorPrintProperties.colors.background);
	...
	void ConsoleScreen::setCellAt(const sf::Vector2u location, const Cell& cell)
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot set cell.\nLocation (" + std::to_string(location.x) + ", " + std::
					to_string(location.y) + ") out of range.");
			return;
		}
	...
	void ConsoleScreen::setValueAt(const sf::Vector2u location, const unsigned int value)
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot set cell value.\nLocation (" + std::to_string(location.x) + ", " + std::
					to_string(location.y) + ") out of range.");
			return;
		}
	...
	void ConsoleScreen::setColorsAt(const sf::Vector2u location, const Color color, const Color backgroundColor)
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot set cell colors.\nLocation (" + std::to_string(location.x) + ", " + std::
					to_string(location.y) + ") out of range.");
			return;
		}
	...
	void ConsoleScreen::setForegroundColorAt(const sf::Vector2u location, const Color color)
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot set cell foreground color.\nLocation (" + std::to_string(location.x) +
	...
	void ConsoleScreen::setBackgroundColorAt(const sf::Vector2u location, const Color backgroundColor)
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot set cell background colors.\nLocation (" + std::to_string(location.x) +
					", " + std::to_string(location.y) + ") out of range.");
			return;
		}
	...
	void ConsoleScreen::setStretchAt(const sf::Vector2u location, const StretchType& stretch)
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot set cell stretch.\nLocation (" + std::to_string(location.x) + ", " + std::
					to_string(location.y) + ") out of range.");
			return;
		}
	...
	void ConsoleScreen::setAttributesAt(const sf::Vector2u location, const CellAttributes& attributes)
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot set cell attributes.\nLocation (" + std::to_string(location.x) + ", " +
					std::to_string(location.y) + ") out of range.");
			return;
		}
	...
	void ConsoleScreen::setAttributesToAt(const sf::Vector2u location, const bool attributeValue,
	                                      const Affect& attributeMask)
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
	...
	ConsoleScreen::Cell ConsoleScreen::getCellAt(const sf::Vector2u location) const
	{
		if (!priv_isCellLocationInRange(location))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot get cell.\nLocation (" + std::to_string(location.x) + ", " + std::
					to_string(location.y) + ") out of range.");
			return Cell();
		}
	...
	void ConsoleScreen::scrollUp(const unsigned int amount)
	{
		...
		for (unsigned int repeat{0}; repeat < amount; ++repeat)
			// lazy way of scrolling multiple times - loop scrolling (entirely by 1 each time)
		{
			...
			for (unsigned int y{0}; y < static_cast<unsigned int>(selectionRectangle.height); ++y)
			{
				for (unsigned int x{0}; x < static_cast<unsigned int>(selectionRectangle.width); ++x)
				{
					...
					if (y < static_cast<unsigned int>(selectionRectangle.height) - 1)
						m_cells[priv_cellIndex({selectionRectangle.left + x, selectionRectangle.top + y})] = m_cells[
							priv_cellIndex({selectionRectangle.left + x, selectionRectangle.top + y + 1})];
					else if (m_do.wrapOnManualScroll)
						m_cells[priv_cellIndex({selectionRectangle.left + x, selectionRectangle.top + y})] = topRow[x];
					else
						priv_clearCell(priv_cellIndex({selectionRectangle.left + x, selectionRectangle.top + y}), true,
						               true);
				}
			}
		}
	...
	void ConsoleScreen::scrollDown(unsigned int amount, sf::IntRect selectionRectangle)
	{
		...
		if (!priv_isSelectionRectangleContainedInScreen(selectionRectangle))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot scroll selection down.\nThe selection rectangle is not valid.");
			return;
		}
			...
			for (unsigned int y{0}; y < static_cast<unsigned int>(selectionRectangle.height); ++y)
			{
				for (unsigned int x{0}; x < static_cast<unsigned int>(selectionRectangle.width); ++x)
				{
					...
					if (cellY > static_cast<unsigned int>(selectionRectangle.top))
						m_cells[priv_cellIndex({selectionRectangle.left + x, cellY})] = m_cells[priv_cellIndex({
							selectionRectangle.left + x, cellY - 1
						})];
					else if (m_do.wrapOnManualScroll)
	...
	void ConsoleScreen::scrollLeft(unsigned int amount, sf::IntRect selectionRectangle)
	{
		...
		if (!priv_isSelectionRectangleContainedInScreen(selectionRectangle))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot scroll selection left.\nThe selection rectangle is not valid.");
			return;
		}
			...
			for (unsigned int x{0}; x < static_cast<unsigned int>(selectionRectangle.width); ++x)
			{
				for (unsigned int y{0}; y < static_cast<unsigned int>(selectionRectangle.height); ++y)
				{
					if (m_do.wrapOnManualScroll && x == 0)
						leftColumn[y] = m_cells[priv_cellIndex(
							{selectionRectangle.left + x, selectionRectangle.top + y})];
					if (x < static_cast<unsigned int>(selectionRectangle.width) - 1)
						m_cells[priv_cellIndex({selectionRectangle.left + x, selectionRectangle.top + y})] = m_cells[
							priv_cellIndex({selectionRectangle.left + x + 1, selectionRectangle.top + y})];
					else if (m_do.wrapOnManualScroll)
						m_cells[priv_cellIndex({
							selectionRectangle.left + x, selectionRectangle.top + y
						})] = leftColumn[y];
					else
						priv_clearCell(priv_cellIndex({selectionRectangle.left + x, selectionRectangle.top + y}), true,
	...
	void ConsoleScreen::scrollRight(unsigned int amount, sf::IntRect selectionRectangle)
	{
		...
		if (!priv_isSelectionRectangleContainedInScreen(selectionRectangle))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot scroll selection right.\nThe selection rectangle is not valid.");
			return;
		}
			...
			for (unsigned int y{0}; y < static_cast<unsigned int>(selectionRectangle.height); ++y)
			{
				for (unsigned int x{0}; x < static_cast<unsigned int>(selectionRectangle.width); ++x)
				{
					...
					if (cellX > static_cast<unsigned int>(selectionRectangle.left))
						m_cells[priv_cellIndex({cellX, selectionRectangle.top + y})] = m_cells[priv_cellIndex({
							cellX - 1, selectionRectangle.top + y
						})];
					else if (m_do.wrapOnManualScroll)
	...
	{
		clear(priv_getModifiedColorFromColorPairUsingSpecifiedColorType(
			{m_cursorPrintProperties.colors.foreground, backgroundColorCommand}, ColorType::Background));
	}
	...
	void ConsoleScreen::setPaletteColor(const Color color, const sf::Color newColor)
	{
		...
		if (!priv_isColorInPaletteRange(color))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot set palette color.\nColor ID (" + std::to_string(color.id) +
					") out of range.");
			return;
		}
	...
	sf::Color ConsoleScreen::getPaletteColor(const Color color) const
	{
		if (!priv_isColorInPaletteRange(color))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot get palette color.\nColor ID (" + std::to_string(color.id) +
					") out of range.");
			return sf::Color::Transparent;
		}

		return m_is.rgbMode
			       ? sf::Color(static_cast<sf::Uint8>(color.id / 65536),
			                   static_cast<sf::Uint8>((color.id % 65536) / 256), static_cast<sf::Uint8>(color.id % 256))
			       : m_palette[color.id];
	}
	...
	void ConsoleScreen::removePaletteColor(const Color color)
	{
		...
		if (!priv_isColorInPaletteRange(color))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot remove palette color.\nColor ID (" + std::to_string(color.id) +
					") out of range.");
			return;
		}
		if (m_palette.size() < 2)
	...
	void ConsoleScreen::paste(const unsigned int index, const sf::Vector2i offset)
	{
		if (!priv_isScreenBufferIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot paste buffer.\nBuffer index (" + std::to_string(index) +
					") out of range.");
			return;
		}
	...
	void ConsoleScreen::removeBuffer(const unsigned int index)
	{
		if (!priv_isScreenBufferIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot remove buffer.\nBuffer index (" + std::to_string(index) +
					") out of range.");
			return;
		}
	...
	void ConsoleScreen::resizeBuffer(const unsigned int index, const sf::Vector2u size)
	{
		if (!priv_isScreenBufferIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot resize buffer.\nBuffer index (" + std::to_string(index) +
					") out of range.");
			return;
		}
	...
	sf::Vector2u ConsoleScreen::getSizeOfBuffer(const unsigned int index) const
	{
		if (!priv_isScreenBufferIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot get size of buffer.\nBuffer index (" + std::to_string(index) +
					") out of range.");
			return sf::Vector2u();
		}

		const Buffer& buffer{m_buffers[index]};
		return sf::Vector2u(static_cast<unsigned int>(buffer.cells.size()) % buffer.width,
		                    static_cast<unsigned int>(buffer.cells.size()) / buffer.width);
	}
	...
	void ConsoleScreen::poke(const unsigned int index, const unsigned int value)
	{
		if (!priv_isCellIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot poke value.\nCell number (" + std::to_string(index) + ") out of range.");
			return;
		}
	...
	void ConsoleScreen::poke(const unsigned int index, const Color& color)
	{
		if (!priv_isCellIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot poke color.\nCell number (" + std::to_string(index) + ") out of range.");
			return;
		}
	...
	void ConsoleScreen::poke(const unsigned int index, const Color& color, const Color& backgroundColor)
	{
		if (!priv_isCellIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot poke colors.\nCell number (" + std::to_string(index) + ") out of range.");
			return;
		}
	...
	void ConsoleScreen::poke(const unsigned int index, const CellAttributes& cellAttributes)
	{
		if (!priv_isCellIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot poke attributes.\nCell number (" + std::to_string(index) +
					") out of range.");
			return;
		}
	...
	void ConsoleScreen::poke(const unsigned int index, const bool attributeValue, const Affect& attributeMask)
	{
		if (!priv_isCellIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot poke attribute.\nCell number (" + std::to_string(index) +
					") out of range.");
			return;
		}
	...
	ConsoleScreen::Cell ConsoleScreen::peek(const unsigned int index) const
	{
		if (!priv_isCellIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot peek cell.\nCell number (" + std::to_string(index) + ") out of range.");
			return Cell();
		}
	...
	ConsoleScreen::Cell& ConsoleScreen::cell(const unsigned int index)
	{
		if (!priv_isCellIndexInRange(index))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot retrieve cell.\nCell number (" + std::to_string(index) +
					") out of range.");
	...
	ConsoleScreen::Cell& ConsoleScreen::bufferCell(const unsigned int bufferIndex, const unsigned int cellIndex)
	{
		if (!priv_isScreenBufferIndexInRange(bufferIndex))
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot retrieve buffer cell.\nBuffer index (" + std::to_string(bufferIndex) +
					") out of range.");
			return fakeCell;
		}
		...
		if (cellIndex >= m_buffers[bufferIndex].cells.size())
		{
			if (m_do.throwExceptions)
				throw Exception(
					exceptionPrefix + "Cannot retrieve buffer cell.\nCell index (" + std::to_string(cellIndex) +
					") out of range.");
	...
	void ConsoleScreen::priv_setVerticesFromCell(unsigned int index, int baseVertex, const bool overLayer)
	{
		...
		if (mainLayer)
		{
			...
			cellColor = getPaletteColor(
				priv_getModifiedColorFromCellUsingSpecifiedColorType(index, ColorType::Foreground).id);
		...
		const bool isCursorBottom{
			(m_cursor.visible) && (m_cursorPrintProperties.stretch == Cs::StretchType::Both) && (m_cursorPrintProperties
				.index + m_mode.x == index)
		};
		...
		if (mainLayer && (isCursor || isCursorBottom))
		{
			...
			{
				cellColor = getPaletteColor(
					priv_getModifiedColorFromColorPairUsingSpecifiedColorType(
						Cs::ColorPair(m_cursor.color.id, bgColor), Cs::ColorType::Foreground));
		...
		const float left{
			offset.x + linearInterpolation(0.f, m_size.x,
		...
		const float top{
			offset.y + linearInterpolation(0.f, m_size.y,
		...
		const float textureTop{
			static_cast<float>(m_textureOffset.y + (textureCell.y + (
				!useCursorValue && cell.stretch == StretchType::Bottom ? 0.5f : 0.f)) * m_tileSize.y)
		};
		const float textureRight{static_cast<float>(m_textureOffset.x + (textureCell.x + 1) * m_tileSize.x)};
		const float textureBottom{
			static_cast<float>(m_textureOffset.y + (textureCell.y + (!useCursorValue && cell.stretch == StretchType::Top
				                                                         ? 0.5f
				                                                         : 1.f)) * m_tileSize.y)
		};
		...
		// main display
		sf::Vertex* const pVertex1{
			(mainLayer ? &m_display[baseVertex] : overLayer ? &m_overDisplay[baseVertex] : &m_underDisplay[baseVertex])
		};
	...
	{
		return (color.id >= 0) && ((m_is.rgbMode && color.id < 16777216L) || (color.id < static_cast<long int>(m_palette
			.size())));
	}

	void ConsoleScreen::priv_clearCell(const unsigned int index, const bool overwriteColor,
	                                   const bool overwriteBackgroundColor)
	{
		const Color color{
			overwriteColor ? m_cursorPrintProperties.colors.foreground : m_cells[index].colors.foreground
		};
		const Color backgroundColor{
			overwriteBackgroundColor ? m_cursorPrintProperties.colors.background : m_cells[index].colors.background
		};
		poke(index, {m_clearValue, ColorPair(color, backgroundColor), StretchType::None, CellAttributes()});
	}

	void ConsoleScreen::priv_clearCell(const unsigned int index, const Color& backgroundColor,
	...
	{
		return Color(
			std::uniform_int_distribution<unsigned long int>(
				0u, m_is.rgbMode ? 16777215L : static_cast<unsigned long int>(m_palette.size()) - 1u)(randomGenerator));
	}
	...
	ConsoleScreen::Color ConsoleScreen::priv_getModifiedColorFromColorPairUsingSpecifiedColorType(
		const ColorPair& colors, const ColorType& colorType)
	{
	...
	ConsoleScreen::Color ConsoleScreen::priv_getModifiedColorFromCellUsingSpecifiedColorType(
		const unsigned int& cellIndex, const ColorType& colorType)
	{
	...
	{
		PrintProperties& printProperties{priv_getCurrentPrintProperties()};
		return (printProperties.colorType == ColorType::Foreground)
			       ? printProperties.colors.foreground
			       : printProperties.colors.background;
	}
	...
	std::string ConsoleScreen::priv_read(unsigned int index, const bool unmapCharacters)
	{
		...
		{
			string += (unmapCharacters
				           ? priv_getCharacterFromCellValue(m_cells[index].value)
	...
	void ConsoleScreen::priv_modifyCellUsingPrintProperties(const unsigned int index, const PrintType& printType,
